...
export function SubscribeButton({ username, productId, isSubscribed }) {
  ...
  return (
    <div style={{ padding: '20px', background: '#fee2e2', color: '#b91c1c', borderRadius: '12px', marginTop: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <strong>You need a subscription to access premium features.</strong>
      <button onClick={handleSubscribe} style={{ background: '#b91c1c', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer' }}>Subscribe Now</button>
    </div>
  );
}
